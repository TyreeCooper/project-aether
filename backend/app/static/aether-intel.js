    ...
    return orig(input,init).then(res=>{
      try{
        ...
        if(url.indexOf("/api/v1/assets/")!==-1){
          res.clone().json().then(data=>{ ensure(); if(data&&data.intel) paint(data.intel); }).catch(()=>{});
        }
      }catch(e){}
      return res;
    });
  };
  if(!document.getElementById("intelCss")){
    const l=document.createElement("link"); l.id="intelCss"; l.rel="stylesheet"; l.href="/static/aether-intel.css"; document.head.appendChild(l);
  }
})();
