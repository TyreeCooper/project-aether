(function(){
  ...
  function renderAsset(){
    ...
    const intel=d.intelligence||{},opp=intel.opportunity_24h||{},cross=intel.cross_asset||{},attr=intel.attribution||{},news=intel.news||{},comm=intel.community||{},risk=intel.risk||{},cap=d.capture||{};
  ...
  function renderSettings(){
    ...
    const intel=s.intelligence||{},intelSources=intel.sources||[],cryptoIntel=intel.crypto_calendar||{};
    $("settingsIntelligence").innerHTML=metric("Macro calendar",intel.macro_calendar_connected?"CONNECTED":"DEGRADED",intel.macro_calendar_connected?"up":"down")+metric("Crypto calendar",String(cryptoIntel.status||"unconfigured").toUpperCase(),cryptoIntel.connected?"up":"")+metric("Crypto events loaded",String(cryptoIntel.events_loaded||0))+metric("Crypto trade influence",cryptoIntel.trade_influence_enabled?"ENABLED":"SHADOW ONLY")+metric("Community coverage",String((intel.community_assets_configured||[]).length)+" assets")+metric("Community trade influence",intel.community_trade_influence_enabled?"ENABLED":"SHADOW ONLY")+metric("Event policy",String(intel.event_policy?.mode||"observe_only").toUpperCase())+metric("Connected sources",String(intelSources.filter(x=>x.status==="connected").length))+metric("Planned sources",String(intelSources.filter(x=>x.status==="planned").length));
