(function(){
  ...
  function renderSettings(){
    ...
    renderOperatorControls();
    const loadChecks=load.checks||{},loadMatrix=load.matrix||{},experiment=load.experiment||{};
    const experimentActive=load.status==="experiment_active";
    $("settingsLoadBadge").textContent=experimentActive?"EXPERIMENT ACTIVE":(load.ready?"READY":"NOT READY");$("settingsLoadBadge").className="badge "+(experimentActive?"amber":load.ready?"good":"bad");
    $("settingsLoadStatus").innerHTML=metric("Release",load.release||"—")+metric("Runtime safety",load.runtime_safe?"PASS":"FAIL",load.runtime_safe?"up":"down")+metric("Entry filters",load.filters_bypassed?"BYPASSED":"NORMAL",load.filters_bypassed?"amber":"up")+metric("Experiment run",experiment.run_id||"—")+metric("Observed books",String(experiment.observed_count??0)+" / "+String(experiment.target_count??12),experiment.complete?"up":"")+metric("Open test books",String((experiment.open_assets||[]).length))+metric("Completed test books",String((experiment.completed_assets||[]).length))+metric("Checks passed",String(Object.values(loadChecks).filter(Boolean).length)+" / "+String(Object.keys(loadChecks).length),load.runtime_safe?"up":"down")+metric("Matrix cells",String(loadMatrix.supported_cells??"—")+" supported · "+String(loadMatrix.unsupported_cells??"—")+" N/A")+metric("Execution test",e.execution_test_mode?"ON":"OFF",e.execution_test_mode?"amber":"up")+metric("Live orders",live.orders_enabled?"ENABLED":"BLOCKED",live.orders_enabled?"down":"up");
