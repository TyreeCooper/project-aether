(function(){
  ...
  function renderFloor(){
    ...
    $("floorRisk").innerHTML=metric("State",String(risk.state||"unknown").toUpperCase())+metric("New entries",risk.new_entries_allowed?"ALLOWED":"RESTRICTED",risk.new_entries_allowed?"up":"down")+metric("Calendar feed",risk.calendar_connected?"CONNECTED":"NOT CONNECTED",risk.calendar_connected?"up":"");
  ...
  function renderAsset(){
    ...
    $("assetCrossAsset").innerHTML=metric("BTC correlation 4h",cross.btc_correlation_4h==null?"—":num(cross.btc_correlation_4h,2))+metric("Relative strength vs BTC",cross.relative_strength_vs_btc_24h_pct==null?"—":pct(cross.relative_strength_vs_btc_24h_pct))+metric("1h change",pct(intel.windows?.["1h"]?.change_pct))+metric("4h change",pct(intel.windows?.["4h"]?.change_pct))+metric("12h change",pct(intel.windows?.["12h"]?.change_pct));
    $("assetAttribution").innerHTML=metric("Status",String(attr.status||"unavailable").toUpperCase())+metric("Verified drivers",String((attr.drivers||[]).length))+metric("Note",attr.note||"—");
    $("assetCommunity").innerHTML=metric("Status",String(comm.status||"unavailable").toUpperCase())+metric("Sentiment",comm.sentiment==null?"—":String(comm.sentiment))+metric("Velocity",comm.velocity==null?"—":String(comm.velocity))+metric("Discussion volume",comm.discussion_volume_ratio==null?"—":num(comm.discussion_volume_ratio,2)+"×")+metric("Note",comm.note||"—");
    $("assetCapture").innerHTML=metric("MFE",cap.mfe_pct==null?"—":pct(cap.mfe_pct))+metric("MAE",cap.mae_pct==null?"—":pct(cap.mae_pct))+metric("Available move",cap.available_move_pct==null?"—":pct(cap.available_move_pct))+metric("Purpose","measure capture, not claim perfect fills");
    $("assetRisk").innerHTML=metric("State",String(risk.state||"unknown").toUpperCase())+metric("New entries",risk.new_entries_allowed?"ALLOWED":"RESTRICTED",risk.new_entries_allowed?"up":"down")+metric("Calendar",risk.calendar_connected?"CONNECTED":"NOT CONNECTED")+metric("Note",risk.note||"—");
