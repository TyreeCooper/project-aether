...
Quotes and 1-minute bars from Yahoo Finance chart/quote endpoints.
These are public last/bid/ask prints — not a broker fill and not invented prices.
Live orders still require that broker's keys later.
"""
...
# If a micro future symbol is thin, fall back to the parent contract.
FALLBACK = {
    "mgc": "GC=F",
    "mcl": "CL=F",
    "us10y": "^TNX",
...
async def fetch_yahoo_quotes() -> dict[str, dict[str, Any]]:
    ...
    for asset in ASSETS:
        aid = str(asset["id"])
        primary = YAHOO.get(aid)
        raw = by_sym.get(primary or "")
...
async def fetch_yahoo_bars(asset_id: str, interval: str = "1m", limit: int = 240) -> list[dict[str, float | int]]:
    ...
    if not symbol:
        asset = BY_ID.get(str(asset_id).lower()) or {}
        symbol = YAHOO.get(str(asset.get("id") or ""))
    if not symbol:
        return []
    range_map = {"1m": "1d", "5m": "5d", "60m": "3mo", "1h": "3mo"}
    rng = range_map.get(interval, "1d")
    url = CHART.format(symbol=symbol)
    async with httpx.AsyncClient(timeout=15.0, headers=HEADERS) as client:
        res = await client.get(url, params={"interval": interval, "range": rng})
        res.raise_for_status()
        payload = res.json()
